...
describe('NodePropertyFactory resources and operations', () => {
	...
	test('createOperationPropertyForResource returns correct operations for Customer', () => {
		const schema = builder.Build();
		const factory = new NodePropertyFactory(schema);
		const operationProp = factory.createN8nOperationPropertyForResource('Customer');

		expect(operationProp).toEqual({
